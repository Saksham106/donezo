...
This pass changes the shared mutation/state/rendering contract used across hot interactions. It intentionally stays inside the existing vanilla-JS + Supabase architecture.
...
- Do not add a framework rewrite, new state-management dependency, service worker mutation queue, or broad offline-write subsystem in this pass.
...
`src/app.js` should use a single state snapshot per render and helpers should accept/pass that state where practical instead of repeatedly calling the repository. The app must not mutate `peekState()` directly except through repository-owned optimistic mutation helpers described below.

This is a targeted performance boundary, not a general store rewrite.

## 2. Repository-local state patching
...
## 3. Optimistic mutation coordinator
...
- For reaction switching, newer intent supersedes older intent. Rapid `🔥 → 👏 → 😂` should settle to `😂`, not replay three full refresh cycles.
...
## 4. Reactions: immediate and last-write-wins
...
- Queue/supersede persistence for that proof.
- Persist the final selected positive reaction using the existing one-positive-reaction database invariant.
- Avoid a full repository `load()` after success.
- If persistence fails, restore the last confirmed reaction state for that proof and show a concise failure toast.

The independent `👎` proof-rejection signal continues to be separate.

## 5. Simple check-ins, replies, and read state
...
## 6. Reconciliation and refresh behavior
...
- Manual refresh remains explicit and authoritative.
...
## 7. Rendering improvements without a framework rewrite
...
## 8. Instrumentation and regression targets
...
- reaction persistence does not call full `load()` on success;
- rapid reaction changes coalesce/last-write-win;
- optimistic rollback restores prior state on failure;
...
Where practical, use deterministic unit tests around the optimistic coordinator rather than fragile wall-clock timing tests.

## 9. Friends list action layout
...
- Optimistic operations store the last confirmed local value needed for rollback.
...
## 12. Supabase and database scope
...
- deploy preview and verify reaction switching, no-proof check-in, reply posting/deleting, Friends invite share flow, add-by-link flow, scroll preservation, and proof viewing;
...
The pass is successful when everyday taps feel immediate even on ordinary mobile network latency: reaction switching changes visually right away, lightweight check-ins/replies/read-state no longer wait for full reloads, unrelated controls do not freeze during small writes, and inviting a friend is a one-tap path from the Friends list into the native share sheet.